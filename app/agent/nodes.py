...
def logistics_node(state: AgentState) -> AgentState:
    """物流查询节点 —— 从用户消息中提取单号，返回物流轨迹信息

    适用于 logistics 意图，直接从 JSON 数据库中查询物流状态。

    Args:
        state: 当前图状态

    Returns:
        更新后的图状态（含物流查询结果的 AI 回复）
    """
    logger.info("[LOGISTICS] 进入物流查询节点...")  # 记录日志

    # 获取最后一条用户消息
    user_message = state["messages"][-1].content if state["messages"] else ""

    # 调用物流查询工具（传入用户消息和 LLM 实例）
    llm = get_llm()  # 获取 LLM 实例
    result_text = handle_logistics_intent(user_message, llm)  # 同步调用物流查询

    # 去掉系统提示前缀，让回复直接对用户友好
    # handle_logistics_intent 返回的文本可能以 "系统提示：" 开头，需要交给 LLM 润色
...
客服回复:"""
        response = llm.invoke(polish_prompt)  # 调用 LLM 润色
        reply_text = response.content.strip()  # 提取润色后文本
    else:
        # 非系统提示（直接查到的物流信息），直接润色为友好回复
